...
fn days_from_civil(year: i64, month: u32, day: u32) -> i64 {
    ...
    let day_of_year =
        (153 * (i64::from(if month > 2 { month - 3 } else { month + 9 })) + 2) / 5
            + i64::from(day)
            - 1;
...
async fn upsert_note(
    ...
    let existing: Option<NoteId> = match &note.joplin_id {
        Some(id) => sqlx::query_scalar("SELECT id FROM notes WHERE uuid = ?")
            .bind(id)
            .fetch_optional(&mut *tx)
            .await?,
        None => None,
...
mod tests {
    ...
    fn quoted_title_keeps_colons_quotes_and_backslashes() {
        ...
        assert_eq!(fm.title.as_deref(), Some("Note: with \"quotes\" and\\slash"));
